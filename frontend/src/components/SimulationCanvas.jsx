...
const SimulationCanvas = ({ 
  ...
  useEffect(() => {
    ...
    // Initially pause the simulation before start is pressed
  ...
  // 3. Synchronize canvas execution state to parent running prop & isPaused toggle
  useEffect(() => {
    if (simRef.current) {
      simRef.current.running = running && !isPaused;
    }
  }, [running, isPaused]);
  ...
  return (
    <div className="relative w-full aspect-[16/10] bg-black rounded-xl sm:rounded-2xl overflow-hidden border border-white/10 shadow-2xl group">
      ...
      <div className="absolute bottom-3 left-3 sm:bottom-4 sm:left-4 flex items-center gap-2 px-2.5 py-1 rounded-full bg-black/60 backdrop-blur-md border border-white/10">
        <div className={`w-2 h-2 rounded-full ${!running ? 'bg-zinc-600' : isPaused ? 'bg-amber-400' : 'bg-emerald-400 animate-ping'}`} />
        <span className="text-[9px] sm:text-[10px] uppercase tracking-widest text-white/70">
          {!running ? 'Simulation Idle' : isPaused ? 'Simulation Paused' : 'Simulation Running'}
        </span>
      </div>
    </div>
  );
