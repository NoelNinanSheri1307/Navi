...
export const BenchmarkCenter = () => {
  // Available algorithms lists
  const availableAlgos = useMemo(() => {
    return rawData.map(d => d.algorithm);
  }, []);

  const [selectedAlgos, setSelectedAlgos] = useState(availableAlgos);
  ...
  const comparedData = useMemo(() => {
    ...
    // Sort by fitness descending (higher is better, or closer to zero since fitness is negative)
    return filtered.sort((a, b) => b.fitness - a.fitness);
  }, [selectedAlgos]);

  // Color mapper helper
  const getAlgoColor = (algo) => {
    if (algo === "ASM") return "#22d3ee"; // Cyan highlight for ASM
    ...
    <Section>
      <Container size="default" className="flex flex-col gap-8">
        <PageHeader
          ...
          description="Standardized multi-run comparison suite executing reference search kernels. Select algorithms to overlay convergence paths and compare delay latency."
        />
        ...
        {comparedData.length === 0 ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            <div className="lg:col-span-8 flex flex-col gap-8">
              ...
              {/* Live Overlay Convergence Curve Line Chart */}
